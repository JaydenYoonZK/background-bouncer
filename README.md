...
- 🔍 **Looks twice when it helps.** Once the first pass has found the subject, the model looks again at just that region, so a subject filling a third of the frame gets the whole canvas rather than a third of it. On every engine: on the processor it roughly doubles the wait for a small subject, and that trade is taken on purpose. Measured on a wide frame: 13% more of the edge resolved as real partial coverage, which is what hair is.
...
1. The photo is resized to the model's canvas, 1024×1024 on a GPU or 384×384 on the processor, and normalized with ImageNet statistics.
...
| `docs/models/birefnet-lite-1024-fp16.onnx.gz` | BiRefNet_lite at 1024×1024 in float16, for the WebGPU path. Stored gzipped (83 MB) because the raw file is past GitHub's 100 MB limit; unpacked in the browser. |
| `docs/models/birefnet-lite-384.onnx` | BiRefNet_lite at 384×384 quantized to int8, for the processor path (about 40 MB gzipped, 67 MB on disk). |

## 📜 Licenses and credits

- Code: [MIT](LICENSE) © Jayden Yoon ZK
- Model: [BiRefNet](https://github.com/ZhengPeng7/BiRefNet) (MIT) by Zheng Peng et al., re-exported to ONNX at 1024×1024 in float16 and at 384×384 in int8
